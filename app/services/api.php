...
class API extends REST {
	...
	private function listNewsInfo(){
		...
		$response = array(
			'status' => 'success', 'count' => $count, 'count_total' => $count_total, 'pages' => $page,
	...
	private function insertGcm(){
        ...
        //Ahmed inser a covid user
        	private function insertAppUserInfo(){
		if($this->get_request_method() != "POST") $this->response('',406);
		...
		$email  = $gcm['email'];
		$regid  = $gcm['regid'];
                $phone_id  = $gcm['phone_id'];
                
                //
             
                
//Query the gcm table to get the id for the user and use it as foreigen key in the new insert
		$column_names = array('device', 'email', 'version', 'regid', 'date_create');
		$table_name = 'gcm';
		$pk = 'id';
		$query="SELECT DISTINCT g.id FROM gcm g WHERE g.regid='$regid' OR ( g.device='$device' AND g.email='$email' )";
		...
		if($r->num_rows > 0){ // update
			$result = $r->fetch_assoc();
			$gcm_id = (int)$result['id'];
//		
                        //Check if the case have been reported before
                        $gcm['gcm_id'] = $gcm_id;
                        
                        $column_names = array('phone_no', 'phone_id', 'country', 'covid_positive','track_name','gcm_id' ,'date_create');
		        $table_name = 'app_users';
		        $pk = 'id';
                        
                        $query="SELECT DISTINCT a.id FROM app_users a WHERE a.gcm_id='$gcm_id' OR a.phone_id='$phone_id' ";
		        $r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
			...
			$id = (int)$result['id'];
                        ...
                        $status = "failed";
			$msg 	= "A record was found in the app_users DB with id =".$id;
                        
                        $resp = array('status' => $status, "msg" => $msg, "data" => array());
			$this->response($this->json($resp),200);
			...
			//$this->post_one($gcm, $pk, $column_names, $table_name);
                        $status = "failed";
			$msg 	= "The user isn't registered in the gcm DB";
	...
	private function sendNotif() {
		...
		$result_arr['failure'] = $result_data->failure;
                ...
                //$this->debug_to_console(' sucess '.$result_arr['success']);
		return $result_arr;
	}
